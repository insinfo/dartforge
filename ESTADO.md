...
| Inferência de corpos, fluxo, constantes | **funcional, com lacunas** (motor reescrito pela especificação, `crates/types/src/inferencia`) | 40/40 negativos do `analyzer`; medido contra o oráculo `package:analyzer` (`tools/oraculo_tipos`): `new_sali/core` 18 avisos e 34 de 634.368 expressões divergentes, `frontend` 31 avisos e 59 de 1.259.011; SDK da fonte (nativo) 18 diagnósticos, os mesmos do analyzer; corpus de conformidade 82/95 programas iguais ao oráculo (ver §2.1) |
...
1. **Lacunas de inferência de tipos** (o `dart analyze` oficial dá 0
   ...
   `docs/FRONTEND-NEW-SALI.md`): `new_sali/core` 13.431 → **18** avisos,
   `frontend` 57.883 → **31**; divergências de tipo estático por
   expressão 137.428 → 34 (core) e 316.756 → 59 (frontend); corpus
