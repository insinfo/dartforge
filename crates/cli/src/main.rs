...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args[0] == "empacotar" {
        #[cfg(feature = "nativo")]
        return empacotar::run(&args[1..]);
        #[cfg(not(feature = "nativo"))]
        return Err("empacotar: compile com --features nativo".into());
    }
