...
A coleta automática ocorre antes de alocar, após um limiar de 256 alocações (ajustado pelo número de objetos vivos). DARTFORGE_GC_STRESS força coleta antes de toda alocação. O resultado da nova alocação precisa ser enraizado antes da próxima. dartforge_gc_collect permite coleta explícita.

## ABI

- dartforge_gc_push_frame() -> i64; dartforge_gc_root(frame:i64, handle:i64); dartforge_gc_pop_frame(frame:i64); dartforge_gc_collect().
...
## Revisão e microbenchmark

Marcação usa bitmap por slot e pilha de trabalho reutilizados, sem hash por objeto. A varredura visita o máximo histórico de slots, inclusive slots livres; não há redução automática da capacidade. Frames procuram do topo para baixo: o caso normal do emissor encontra imediatamente o frame corrente, mas registrar em um ancestral custa profundidade. Raízes duplicadas ainda são retidas e processadas; funções longas e loops podem consumir memória linear no número de registros. Slots fixos com análise de liveness são melhoria futura, sem mudança silenciosa da ABI.
