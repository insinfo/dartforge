...
fn o_que_geramos_e_igual_ao_oficial() {
    ...
    if !oraculo.is_dir() {
        eprintln!("sem oráculo em {}; rode scripts/corpus-ngdart.ps1", oraculo.display());
        return;
    }
    ...
    for (caminho, f) in nossa.iter() {
        ...
        let Ok(esperado) = std::fs::read_to_string(&oficial) else { continue };
        conferidos += 1;
        if esperado.replace("\r\n", "\n") != f.conteudo.as_ref() {
            diferentes.push((caminho.clone(), esperado.replace("\r\n", "\n"), f.conteudo.to_string()));
        }
    }
    ...
    assert!(diferentes.is_empty(), "{} arquivo(s) diferentes do oficial", diferentes.len());
    println!(
    ...
    assert!(conferidos >= 2, "o corpus tem de conferir pelo menos as formas cobertas");
}
...
fn todo_arquivo_do_corpus_tem_oraculo() {
    ...
    for e in std::fs::read_dir(&src).expect("lib/src").flatten() {
        ...
        let esperado = oraculo.join(
            caminho_do_template(&p).file_name().unwrap_or_default(),
        );
        if !esperado.is_file() {
            faltando.push(nome);
        }
    }
    assert!(faltando.is_empty(), "sem oráculo: {faltando:?} — rode scripts/corpus-ngdart.ps1");
}
