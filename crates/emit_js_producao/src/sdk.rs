    ...
    while i < b.len() {
        match b[i] {
            ...
            b'"' | b'\'' => {
                ...
                // citando.
                if !s.is_empty() && s.len() < 64 && inicio_de_ident(s.as_bytes(), 0) && s.bytes().all(|c| c.is_ascii_alphanumeric() || matches!(c, b'_' | b'
            }
            _ => i += 1,
        }
    }
...
fn partir_classe(src: &str, ini: usize, fim: usize, sym: &str, fatias: &mut Vec<Fatia>, grupo: &mut u32) -> bool {
    ...
    for (a, b) in membros {
        ...
        match nome_do_membro(txt) {
            // Sem nome reconhecido, o membro vive com a classe: conservador.
            Some(n) => req.push(format!("sel:{n}")),
            None => {}
        }
    ...
    for (ini, fim) in varredura::declaracoes(src) {
                    ...
                    if entradas.len() >= 2 {
                        ...
                        for (a, b) in entradas {
                            ...
                            let (gat, req) = match (&chave, e_ct, e_classe) {
                                ...
                                (Some(k), false, true) if por_membro => (vec![], vec![alvo_s.clone(), format!("sel:{k}")]),
                        ...
                        if let Some(fecha) = varredura::fecha_colchete(src, abre) {
                            ...
                            if !entradas.is_empty() && por_membro {
                                ...
                                for (a, b) in entradas {
                                    ...
                                    let corte = a + txt.len() - usize::from(txt.ends_with(','));
                                    let req = if nome.is_empty() { vec![alvo.clone()] } else { vec![alvo.clone(), format!("sel:{nome}")] };
        ...
        if let Some((a, i)) = primeiro_ident(tt) {
            if e_lib(a) && tt.as_bytes().get(i) == Some(&b'[') {
                ...
                referencias(&tt[..i + 40.min(tt.len() - i)], &mut refs);
                if let Some(s) = refs.into_iter().find(|s| s.starts_with(a) && s.contains('|')) {
                    fatias.push(simples(vec![s]));
                    continue;
                }
            }
        }
