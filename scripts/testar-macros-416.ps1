...
$caso = Join-Path $raiz 'corpus/macros/416_definicao_funcao'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'hospedeiro não materializou o 416' }
}
finally {
    Pop-Location
}
$esperado = [IO.File]::ReadAllText(
    ...
    "augment library 'package:caso_definicao_funcao/calculo.dart';", "augment library 'calculo.dart';").Replace(
    [Environment]::NewLine, [string][char]10)
$obtido = [IO.File]::ReadAllText((Join-Path $caso 'lib/calculo.macro.dart')).Replace(
    [Environment]::NewLine, [string][char]10)
if ($obtido -cne $esperado) { throw '416: augmentation do hospedeiro divergiu byte a byte do CFE' }
Write-Host "416: augmentation de $($obtido.Length) caracteres igual ao CFE byte a byte"
