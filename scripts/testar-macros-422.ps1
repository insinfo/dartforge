...
$caso = Join-Path $raiz 'corpus/macros/422_macro_biblioteca'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'hospedeiro não materializou o 422' }
}
finally {
    Pop-Location
}
$esperado = [IO.File]::ReadAllText(
    ...
    "augment library 'package:caso_macro_biblioteca/caso.dart';", "augment library 'caso.dart';").Replace(
    [Environment]::NewLine, [string][char]10)
$obtido = [IO.File]::ReadAllText((Join-Path $caso 'lib/caso.macro.dart')).Replace(
    [Environment]::NewLine, [string][char]10)
if ($obtido -cne $esperado) { throw '422: augmentation do hospedeiro divergiu byte a byte do CFE' }
Write-Host "422: augmentation de $($obtido.Length) caracteres igual ao CFE byte a byte"
