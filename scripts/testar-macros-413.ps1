...
$caso = Join-Path $raiz 'corpus/macros/413_argumentos_nomeados'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'hospedeiro não materializou o 413' }
}
finally {
    Pop-Location
}
$esperado = [IO.File]::ReadAllText(
    ...
    "augment library 'package:caso_etiquetas/pedido.dart';", "augment library 'pedido.dart';").Replace(
    [Environment]::NewLine, [string][char]10)
$obtido = [IO.File]::ReadAllText((Join-Path $caso 'lib/pedido.macro.dart')).Replace(
    [Environment]::NewLine, [string][char]10)
if ($obtido -cne $esperado) { throw '413: augmentation do hospedeiro divergiu byte a byte do CFE' }
Write-Host "413: augmentation de $($obtido.Length) caracteres igual ao CFE byte a byte"
