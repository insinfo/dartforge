...
$caso = Join-Path $raiz 'corpus/macros/421_macro_part'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'hospedeiro não materializou o 421' }
}
finally {
    Pop-Location
}
$esperado = [IO.File]::ReadAllText(
    ...
    "augment library 'package:caso_macro_part/caso.dart';", "augment library 'caso.dart';").Replace(
    [Environment]::NewLine, [string][char]10)
$obtido = [IO.File]::ReadAllText((Join-Path $caso 'lib/caso.macro.dart')).Replace(
    [Environment]::NewLine, [string][char]10)
if ($obtido -cne $esperado) { throw '421: augmentation do hospedeiro divergiu byte a byte do CFE' }
Write-Host "421: augmentation de $($obtido.Length) caracteres igual ao CFE byte a byte"
