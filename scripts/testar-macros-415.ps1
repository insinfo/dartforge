...
$caso = Join-Path $raiz 'corpus/macros/415_variavel_topo'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'hospedeiro não materializou o 415' }
}
finally {
    Pop-Location
}
$esperado = [IO.File]::ReadAllText(
    ...
    "augment library 'package:caso_variavel_topo/calculo.dart';", "augment library 'calculo.dart';").Replace(
    [Environment]::NewLine, [string][char]10)
$obtido = [IO.File]::ReadAllText((Join-Path $caso 'lib/calculo.macro.dart')).Replace(
    [Environment]::NewLine, [string][char]10)
if ($obtido -cne $esperado) { throw '415: augmentation do hospedeiro divergiu byte a byte do CFE' }
Write-Host "415: augmentation de $($obtido.Length) caracteres igual ao CFE byte a byte"
